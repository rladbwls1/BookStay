...
public class hotelDAO extends OracleDB {
	...
	public ArrayList<hotelDTO> hotelList(int sel, String check, String title, String checkin, String checkout,
			int[] adult, int[] kid, int start, int end) {
		...
		if (sel == 5) {
			sql = "SELECT h.*, r.average_value " + "FROM hotel h " + "LEFT JOIN ( "
					+ "    SELECT ref, ROUND(AVG(jumsu), 1) AS average_value " + "    FROM review "
					+ "    GROUP BY ref " + ") r ON h.ref = r.ref " + "WHERE (h.title LIKE ? OR h.address LIKE ?) "
					+ "AND h.type IN (";
			for (int i = 0; i < typeArray.length; i++) {
				sql += (i == 0 ? "?" : ", ?");
			}
			sql += ") " + "AND h.adultmax >= ? " + "AND h.kidmax >= ? " + "AND h.re_step = 0 "
					+ "AND ROWNUM BETWEEN ? AND ? " + "ORDER BY r.average_value DESC NULLS LAST";
		} else {

			sql = "select * from " + " (select b.* , rownum r from "
					+ " (select * from hotel where (title like ? or address like ?) " + " and type in (";
			for (int i = 0; i < typeArray.length; i++) {
				sql += (i == 0 ? "?" : ", ?");
			}
	...
	public ArrayList<hotelDTO> calendar() {
		...
		try {
			String sql = "SELECT horder.*, hotel.title " + " FROM horder " + " JOIN hotel ON horder.ref = hotel.num";
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public ArrayList<hotelDTO> HotelBlock(String chkin, String chkout) {
		...
		conn = getConnection();
		try {
			String sql = "select * " + " from horder " + "where (checkin < to_date(?) AND checkout > to_date(?))";
			pstmt = conn.prepareStatement(sql);
